...
namespace SmartBox.Console.Dao
{
    ...
    public class SMC_BUAUserSyncToInsideDao : BaseDao<SMC_BUAUserSyncToInside>
    {

        public SMC_BUAUserSyncToInsideDao(string key)
            : base(key)
        {
        ...
        {
            string columns = @"busi_id,user_uid,user_name,sync_bat_no,sync_time,sync_status,description";
            string sqlorder;
            ...
            SelectPagnationEx r = base.SelectPaginationEx("SMC_BUAUserSyncToInside", columns, view.PageIndex + 1, view.PageSize, sqlorder, sWhere, "");
            view.RecordCount = r.RecordCount;

            JsonFlexiGridData data = BaseDao<object>.ConvertJosnFlexGridData(r.Result.Tables[0], view, "busi_id");
            return data;
        }
        ...
        public void DeleteList(List<int> busi_ids)
        {
            string ids = "";
            foreach (int i in busi_ids) {
                ids += i.ToString() + ",";
            }
            string sql = "delete from SMC_BUAUserSyncToInside where busi_id in (" + ids.TrimEnd(",".ToCharArray()) + ")";
...
//        }
        ...
        public bool ExistsByCode(string busi_id)
        {
            string sql = "select count(busi_id) from SMC_BUAUserSyncToInside where busi_id=@busi_id";
            Hashtable pars = new Hashtable();
            pars.Add("busi_id", busi_id);
        ...
        {
            string sql = "select isnull(max(sync_bat_no),0) bat_no from dbo.SMC_BUAUserSyncToInside";
