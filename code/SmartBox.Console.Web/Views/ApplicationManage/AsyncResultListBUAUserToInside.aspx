    ...
    <script language="javascript" type="text/javascript">
        function loadData() {
            ...
            var option = {
                height: gh,
                width: mainWidth,
                url: '<%=Url.Action("QueryAsyncResultListBUAUserToInside")%>?sync_bat_no=<%=Request.QueryString["sync_bat_no"]%>&sync_time_start=<%=Request.QueryString["sync_time_start"]%>&sync_time_end=<%=Request.QueryString["sync_time_end"]%>&sync_status=<%=Request.QueryString["sync_status"]%>&userName=' + userName,
                colModel: [
                    { display: '操作', name: 'id', width: 25, sortable: false, align: 'center', process: function (value, pid) {
                        var op = new Array();
                        op.push("<input type='checkbox' id=" + pid + " \/>");
                        return op.join('&nbsp;&nbsp;')
                    }
                    },
                    { display: '用户账号', name: 'user_uid', width: 140, sortable: false, align: 'left' },
                    { display: '用户行名', name: 'user_name', width: 100, sortable: false, align: 'left' },
        ...
        $("document").ready(function () {
            ...
            $('div#Search').click(function () {
                ...
                var url = '<%=Url.Content("~/ApplicationManage/AsyncResultListBUAUserToInside")%>?sync_bat_no=' + batNo + '&userName=' + userName + '&sync_status=' + sync_status + '&sync_time_start=' + sync_time_start + '&sync_time_end=' + sync_time_end;
                window.location = url;
            });
            ...
            $("div#Save").click(function () {
                ...
                $.ajax({
                    type: "POST",
                    url: '<%=Url.Action("DeleteBUAUserAsyncToInsideResult") %>',
                    data: { ids: ids },
                    dataType: "json",
...
<body>
    ...
    <div class="cHead">
            <div class="ftitle">
                <span id="departmentName">BUA用户同步内网结果查询</span>
            </div>
        </div>   
