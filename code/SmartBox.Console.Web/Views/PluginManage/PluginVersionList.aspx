...
<head id="Head1" runat="server">
    ...
    <script type="text/javascript">
        ...
        function toolbarItem_onclick(cmd, grid) {
            if (cmd == "Add") {
                window.location = '<%=Url.Action("UploadVersionFile") %>/?IsAdd=1';
            }
        ...
        function openWindowsclick(Id, IsAdd) {
            var url = "";
            if (IsAdd == "true")//若是升级
                url = '<%=Url.Action("UploadVersionFile") %>/' + escape(Id) + '/?IsAdd=1';
            else
                url = '<%=Url.Action("UploadVersionFile") %>/' + escape(Id) + '/?IsAdd=0';
            window.location = url;
...
<body>
    <div style="padding: 1px;">
        <%--<div class="cHead">
            <div class="ftitle">
                <span id="departmentName">插件版本管理</span>
            </div>
        </div>--%>
