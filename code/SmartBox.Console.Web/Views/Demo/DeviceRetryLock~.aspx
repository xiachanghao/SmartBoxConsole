...
<div id="_layout" class="easyui-layout" data-options="fit:true" style="">
    <div data-options="region:'north',split:false" style="height:163px;overflow:hidden;border:0px solid #DDDDDD;">
        <div class="easyui-panel cHead" data-options="" style="display:;font-size:12px;color:#528FB6;text-align: left; border:1px solid #DDDDDD;padding-left:5px;">
            <img src="../../themes/default/images/flexigrid/grid.png" /><span>重试锁定>>设备重试锁定</span>
        </div>
        ...
        <table border="0" cellspacing="0" cellpadding="0">
            <tbody>
                ...
                <tr>
                    ...
                    <td><input id="tbTimeStart" name="tbTimeStart" readonly="readonly"  type="text"  class="Wdate" onClick="WdatePicker()"/>-<input id="tbTimeEnd" name="tbTimeEnd" readonly="readonly"  type="text"  class="Wdate" onClick="WdatePicker()"/>
                        <span style="display:none;"><select id="selState" class="easyui-combobox" data-options="panelHeight:'auto'" name="state" style="width:184px;">
...
<a id="btn_upload" onclick="javascript:return UnLockSelectedDevice();" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true">解除锁定</a>
<%--<a onclick="javascript:return NotPassSelected();" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-remove',plain:true">拒绝通过</a>--%>
     ...
     <table id="grid" class="easyui-datagrid" style="width:100%;border-top:1px solid #DDDDDD"
					data-options="url:'<%=Url.Content("~/Demo/GetDeviceRetryLock") %>',idField:'id',method:'get',displayMsg:'当前{from}-{to}，共 {total}',loadMsg:'正在加载...',rownumbers:true,checkOnSelect:false,selectOnCheck:true,singleSelect:false,pagination:true,border:false,fit:true,fitColumns:true,toolbar: '#tb',loadFilter: function (data) {
                            for (var i = 0; i < data.rows.length; ++i) {
                                data.rows[i].locktime2 = parseDate(data.rows[i].locktime);
                                switch(data.rows[i].status) {
                                    case 0:
                                        data.rows[i].status_text = '正常';
                                        break;
                                    case 1:
                                        data.rows[i].status_text = '已锁定';
                                        break;
                                    case 2:
				...
				<thead>
					<tr>
                        ...
                        <th data-options="field:'deviceid'" width="120">设备号</th>
                        <th data-options="field:'model'" width="50">设备类型</th>
                        <th data-options="field:'description'" width="50">描述</th>
                        <th data-options="field:'locktime2'" width="50">锁定时间</th>
                        <th data-options="field:'status_text'" width="30">状态</th>
						<th data-options="field:'id2',formatter: function(value,row,index){
				return '<a href=\'#\' onclick=\'javascript:return UnLockDevice(&quot;'+row.id+'&quot;);\'>解除锁定</a>' + 
            ...
            <script type="text/javascript">
                ...
                function UnLockSelectedDevice() {
                    var rows = $('#grid').datagrid('getChecked');
                    if (rows.length < 1)
                        return;
                    $.messager.confirm('确认', '确定要解锁所勾选的设备吗?', function (r) {
                        if (r) {
                            var ids = '';
                ...
                function UnLockDevice(id) {
                    $.messager.confirm('确认', '确定要解锁设备吗?', function (r) {
                        if (r) {
                            $.ajax({
                                type: "POST",
