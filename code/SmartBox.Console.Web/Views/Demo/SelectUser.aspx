...
<div id="_layout" class="easyui-layout" data-options="fit:true" style="">
    <div data-options="region:'north',split:false" style="height:163px;overflow:hidden;border:0px solid #DDDDDD;">
        <div class="easyui-panel cHead" data-options="" style="display:;font-size:12px;color:#528FB6;text-align: left; border:1px solid #DDDDDD;padding-left:5px;">
            <img src="../../themes/default/images/flexigrid/grid.png" /><span>选择用户</span>
        </div>
...
<a id="btn_upload" onclick="javascript:return PassSelected();" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true">保 存</a>
<%--<a onclick="javascript:return NotPassSelected();" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-remove',plain:true">拒绝通过</a>--%>
     ...
     <table id="grid" class="easyui-datagrid" style="width:100%;border-top:1px solid #DDDDDD"
					data-options="url:'<%=Url.Content("~/Demo/GetSelectUser") %>',idField:'id',method:'get',displayMsg:'当前{from}-{to}，共 {total}',loadMsg:'正在加载...',rownumbers:true,checkOnSelect:false,selectOnCheck:true,singleSelect:false,pagination:true,border:false,fit:true,fitColumns:true,toolbar: '#tb',loadFilter: function (data) {
                            for (var i = 0; i < data.rows.length; ++i) {
                                data.rows[i].u_auth_submit_time2 = parseDate(data.rows[i].u_auth_submit_time);
                                switch(data.rows[i].u_enable_status) {
                                    case 0:
                                        data.rows[i].status_text = '禁用';
                                        break;
                                    case 1:
                                        data.rows[i].status_text = '启用';
                                        break;
                                    case 2:
				...
				<thead>
					<tr>
                        ...
                        <%--<th data-options="field:'u_auth_submit_time2'" width="50">提交时间</th>
                        <th data-options="field:'status_text'" width="30">状态</th>--%>
						<%--<th data-options="field:'id2',formatter: function(value,row,index){
				return '<a href=\'#\' onclick=\'javascript:return Pass(&quot;'+row.useruid+'&quot;,&quot;'+row.username+'&quot;);\'>审核通过</a>' + 
            ...
            <script type="text/javascript">
                ...
                function PassSelected() {
                    ...
                    if (rows.length < 1)
                        return;
                    $.messager.confirm('确认', '确定要选择所勾选的用户吗?', function (r) {
                        if (r) {
                            var ids = '';
