...
<div id="_layout" class="easyui-layout" data-options="fit:true" style="">
    <div data-options="region:'north',split:false" style="height:195px;overflow:hidden;border:0px solid #DDDDDD;">
        <div class="easyui-panel cHead" data-options="" style="display:;font-size:12px;color:#528FB6;text-align: left; border:1px solid #DDDDDD;padding-left:5px;">
            <img src="../../themes/default/images/flexigrid/grid.png" /><span>设备管理>>设备挂失</span>
        </div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
		                ...
		                <option value="ENABLE">已挂失</option>
		                <option value="DISABLE">未挂失</option>
                        </select>
                    </td>
                    <td>单位：</td>
...
<a id="btn_upload" onclick="javascript:return UploadDll();" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true">解除挂失</a>
<a onclick="javascript:return DeleteDll();" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-remove',plain:true">挂失</a>
<a onclick="javascript:return DeviceLostAdd();" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-remove',plain:true">新增挂失</a>
</div>
     <table id="grid" class="easyui-datagrid" style="width:100%;border-top:1px solid #DDDDDD"
					data-options="url:'<%=Url.Content("~/Demo/GetDeviceLostList") %>',idField:'id',method:'get',displayMsg:'当前{from}-{to}，共 {total}',loadMsg:'正在加载...',rownumbers:true,checkOnSelect:false,selectOnCheck:true,singleSelect:false,pagination:true,border:false,fit:true,fitColumns:true,toolbar: '#tb'",loadFilter: function (data) {
                            for (var i = 0; i < data.rows.length; ++i) {
                                //data.rows[i].pd_status_text = data.rows[i].pd_status ? '已启用' : '已禁用';
                                //data.rows[i].pdid = data.rows[i].pd_id;
                                data.rows[i].clienttype = 'pad/android';
                            }
                            return data;
                        }>
				<thead>
					<tr>
                        ...
                        <th data-options="field:'deviceno'" width="180">设备号</th>
                        <th data-options="field:'clienttype'" width="50">设备类型</th>
                        <th data-options="field:'lost_time'" width="50">挂失时间</th>
                        <th data-options="field:'unlost_time'" width="50">解除挂失时间</th>
                        <th data-options="field:'desc'" width="50">描述</th>
                        <th data-options="field:'device_status'" width="30">状态</th>
						<th data-options="field:'id2',formatter: function(value,row,index){
				return '<a href=\'#\' onclick=\'javascript:return ModifyEntity('+value+');\'>解除挂失</a>' + 
                '&nbsp;<a href=\'#\' onclick=\'javascript:return DeviceLostAdd('+value+')\'>挂失</a>' + 
            ...
            <script type="text/javascript">
                ...
                function DeviceLostAdd(id) {
                    var url = '<%=Url.Content("~/") %>Demo/DeviceLostAdd?pd_id=' + id;
                    $('#wif')[0].src = url;
                    $('#w').window('open');
