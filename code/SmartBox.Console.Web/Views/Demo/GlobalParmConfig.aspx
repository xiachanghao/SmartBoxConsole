        ...
        <div class="table_box_data table_box_data_model_details">
            <table  cellspacing="0" cellpadding="0">
                ...
                <tr>
                    ...
                    <tr style="display:;">
                        <th width="30%">设备审核拒绝通过后是否禁止用户使用该设备：</th>
                        <td class=""><select id="sel_disable_device_after_notpass_device" class="easyui-combobox" data-options="panelHeight:'auto'" name="state" style="width:151px;">
		                <option value="1">是，禁止用户使用该设备</option>
		                <option value="0">否，不禁止用户使用该设备</option>
                        </select></td>
                    </tr>
                    ...
                    <tr style="display:none;">
                        <th width="30%">用户锁定后自动解锁时间间隔：</th>
                        <td class=""><input value="24" id="tb_user_unlock_auto_hours" />小时</td>
                    </tr>
                    <tr style="display:none;">
                        <th width="30%">用户锁定计算时间间隔：</th>
                        <td class=""><input value="24" id="tb_lock_user_farto_lasttime_hours" />小时内多次输入错误密码，将导致帐户被锁定</td>
                    </tr>
                    <tr style="display:none;">
                        <th width="30%">用户锁定计算方式：</th>
                    ...
                    <tr style="display:none;">
                        ...
                        <td class=""><input value="3" id="tb_allowed_wrong_times_when_login"/>次输入错误密码，将导致帐户被锁定</td>
                    </tr>
                    <tr style="display:none;">
                        <th width="30%">发送日期：</th>
                    ...
                    <tr style="display:;">
                        <th width="30%">bua应用授权地址：</th>
                        <td class=""><input id="tb_bua_auth_url" /></td>
                    </tr>
                    <tr style="display:none;">
                        <th width="30%">主机IP：</th>
                        <td class=""><input /></td>
...
function closeWin() {
        ...
        for (var i = 0; i < globalparm.length; ++i) {
            ...
            switch (o.ConfigKey) {
                ...
                case "user_default_status":
                    ...
                    break;
                case "bua_auth_url":
                    $('#tb_bua_auth_url').val(o.ConfigValue);
                    break;
                case "namelist_black_or_white":
                    $('#sel_namelist_black_or_white').combobox('select', o.ConfigValue);
                    break;
                case "disable_device_after_notpass_device":
                    $('#sel_disable_device_after_notpass_device').combobox('select', o.ConfigValue);
                    break;
                case "user_unlock_auto_enabled":
                    $('#sel_user_unlock_auto_enabled').combobox('select', o.ConfigValue);
                    break;
                case "lock_count_mode":
                    $('#sel_lock_count_mode').combobox('select', o.ConfigValue);
                    break;
                case "user_unlock_auto_hours":
                    $('#tb_user_unlock_auto_hours').val(o.ConfigValue);
                    break;
                case "lock_user_farto_lasttime_hours":
    ...
    $(document).ready(function () {
        ...
        $('#btnSave').click(function () {
            ...
            o.disable_device_after_notpass_device = $('#sel_disable_device_after_notpass_device').combobox('getValue');
            ...
            o.bua_auth_url = $('#tb_bua_auth_url').val();

            $.ajax({
                type: "POST",
                url: '<%=Url.Action("GlobalParamPost") %>',
                data: o,
                dataType: "json",
                success: function (data) {
