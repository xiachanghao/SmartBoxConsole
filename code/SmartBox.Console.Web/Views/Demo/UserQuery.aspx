...
<div id="_layout" class="easyui-layout" data-options="fit:true" style="">
    <div data-options="region:'north',split:false" style="height:164px;overflow:hidden;border:0px solid #DDDDDD;">
        <div class="easyui-panel cHead" data-options="" style="display:;font-size:12px;color:#528FB6;text-align: left; border:1px solid #DDDDDD;padding-left:5px;">
            <img src="../../themes/default/images/flexigrid/grid.png" /><span>用户管理>>用户比对</span>
        </div>
...
<a id="btn_upload" onclick="javascript:return SyncSelected();" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true">同步</a>
...
<a onclick="javascript:return DeleteDll();" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true">删除用户</a>--%>
     ...
     <table id="grid" class="easyui-datagrid" style="width:100%;border-top:1px solid #DDDDDD"
					data-options="url:'<%=Url.Content("~/Demo/GetUserComparerationList") %>',idField:'U_ID',method:'get',displayMsg:'当前{from}-{to}，共 {total}',loadMsg:'正在加载...',rownumbers:true,checkOnSelect:false,selectOnCheck:true,singleSelect:false,pagination:true,border:false,fit:true,fitColumns:true,toolbar: '#tb',
                    loadFilter: function (data) {  
                        for (var i = 0; i < data.rows.length; ++i) {  
                            data.rows[i].u_need_sync_compare_time2 = parseDate(data.rows[i].u_need_sync_compare_time);
				...
				<thead>
					<tr>
                        ...
                        <th data-options="field:'u_need_sync_compare_time2',sortable:true" width="30">同步比较时间</th>                       
						<th data-options="field:'U_UID2',formatter: function(value,row,index){
				return '<a href=\'#\' onclick=\'javascript:return SyncUser(&quot;'+row.U_UID+'&quot;,&quot;'+row.U_NAME+'&quot;);\'>同步</a>' + 
                '&nbsp; '+ 
            ...
            <script type="text/javascript">
                ...
                function SyncUser(uid, uname) {
                    $.messager.confirm('确认', '确定要同步用户' + uname + '吗?', function (r) {
                        if (r) {
                            $('#w2').window('open');
                            $.ajax({
                                type: "POST",
                ...
                var page = 0;
                var sortStr = 'u_need_sync_compare_time desc';
                    ...
                    pager.pagination({
                        onSelectPage: function (pageNum, pageSize) {
                            ...
                            $('#grid').datagrid('load', {
                                ...
                                uname: $('#tbUName').val(),
                                u_unitcode: unitCode,
                                u_compare_time_start: $('#tbTimeStart').val(),
                                u_compare_time_end: endTime,
                                orderby: sortStr,
                                pageIndex: page - 1,
