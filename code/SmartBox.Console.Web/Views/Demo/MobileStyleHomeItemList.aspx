...
<div id="_layout" class="easyui-layout" data-options="fit:true" style="">
    <div data-options="region:'north',split:false" style="height:29px;overflow:hidden;border:0px solid #DDDDDD;">
        <div class="easyui-panel cHead" data-options="" style="display:;font-size:12px;color:#528FB6;text-align: left; border:1px solid #DDDDDD;padding-left:5px;">
            <img src="../../themes/default/images/flexigrid/grid.png" /><span>界面>>移动界面布局设置列表</span>
        </div>
...
<a id="btn_upload" onclick="javascript:return AddStyleItem();" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true">添加布局设置</a>
</div>
     <table id="grid" class="easyui-datagrid" style="width:100%;border-top:1px solid #DDDDDD"
					data-options="url:'<%=Url.Content("~/Demo/GetMobileStyleHomeItemList") %>',idField:'ID',method:'get',displayMsg:'当前{from}-{to}，共 {total}',loadMsg:'正在加载...',rownumbers:true,checkOnSelect:false,selectOnCheck:true,singleSelect:false,pagination:true,border:false,fit:true,fitColumns:true,toolbar: '#tb',loadFilter: function (data) { for (var i = 0; i < data.rows.length; ++i) {  data.rows[i].App4AIID2 = data.rows[i].App4AIID;  }    return data; },onClickRow:function(rowIndex, rowData) {currData=rowData;}">
				<thead>
					<tr>
                        <th data-options="field:'StyleID',checkbox:true"></th>
						<th data-options="field:'App4AIID'" width="50">代码</th>
                        <th data-options="field:'DispalyName'" width="50">显示名称</th>
                        <th data-options="field:'Image'" width="120">客户端类型</th>
                        <th data-options="field:'Seq'" width="120">客户端类型</th>
						<th data-options="field:'App4AIID2',formatter: function(value,row,index){
				return '<a href=\'#\' onclick=\'javascript:return ModifyEntity('+value+');\'>修改</a>' + 
                '&nbsp;<a href=\'#\' onclick=\'javascript:return DelItem('+value+','+row.App4AIID+')\'>删除</a>' + 
            ...
            <script type="text/javascript">
                var currData = {};
                function ModifyEntity(App4AIID) {
                    var url = '<%=Url.Content("~/") %>demo/MobileStyleHomeItemAdd?App4AIID=' + App4AIID + '&StyleID=<%=Request.QueryString["ID"] %>';
                    $('#wif')[0].src = url;
                    $('#w').window('open');
                    return false;
                }
                ...
                function AddStyleItem() {
                    var url = '<%=Url.Content("~/") %>Demo/MobileStyleHomeItemAdd?StyleID=<%=Request.QueryString["id"] %>&App4AIID=0';
                    $('#wif')[0].src = url;
                    $('#w').window('open');
                    return false;
                }
                ...
                function DelItem(Id, _App4AIID) {

                    $.ajax({
                        type: "POST",
                        url: '<%=Url.Action("StyleItemDeleteItem") %>',
                        data: { App4AIID: Id, StyleID: <%=Request.QueryString["id"] %> },
                        dataType: "json",
...
//                }
                ...
                $(document).ready(function () {
                    ...
                    var opts = dg.datagrid('options');
                    var _styleId = '<%=Request.QueryString["styleid"] %>';
...
//                }
    ...
    <div id="w" class="easyui-window" title="插件" closed="true" modal="true" data-options="minimizable:false,collapsible:false,maximizable:false,onClose:function(){$('#wif').attr('src', ''); return false}" style="width:690px;height:420px;padding:3px;">
