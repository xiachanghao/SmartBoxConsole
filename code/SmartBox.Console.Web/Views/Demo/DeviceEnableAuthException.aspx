...
<div id="_layout" class="easyui-layout" data-options="fit:true" style="">
    <div data-options="region:'north',split:false" style="height:30px;overflow:hidden;border:0px solid #DDDDDD;">
        <div class="easyui-panel cHead" data-options="" style="display:;font-size:12px;color:#528FB6;text-align: left; border:1px solid #DDDDDD;padding-left:5px;">
            <img src="../../themes/default/images/flexigrid/grid.png" /><span>设备管理>>设备启用审核例外</span>
        </div>
...
<a id="btn_upload" onclick="javascript:return AddEnableException();" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true">添加</a>
<a onclick="javascript:return DeleteSelected();" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-remove',plain:true">删除</a>

</div>
</span>

     <table id="grid" class="easyui-datagrid" style="width:100%;border-top:1px solid #DDDDDD"
					data-options="url:'<%=Url.Content("~/Demo/GetDeviceEnableAuthorizationException") %>',idField:'id',method:'get',displayMsg:'当前{from}-{to}，共 {total}',loadMsg:'正在加载...',rownumbers:true,checkOnSelect:false,selectOnCheck:true,singleSelect:false,pagination:true,border:false,fit:true,fitColumns:true,toolbar: '#tb',loadFilter: function (data) {
				...
				//return '<a href=\'#\' onclick=\'javascript:return Pass(&quot;'+row.id+'&quot;);\'>添加</a>' + 
                return '<a href=\'#\' onclick=\'javascript:return DeleteEnableDeviceException(&quot;'+row.id+'&quot;);\'>删除</a>' + 
            ...
            <script type="text/javascript">
                ...
                function DeleteSelected() {
                    ...
                    if (rows.length < 1)
                        return;
                    $.messager.confirm('确认', '确定要删除所勾选的例外吗?', function (r) {
                        if (r) {
                            var ids = '';
                            for (var i = 0; i < rows.length; ++i) {
                                ids += rows[i].id;
                            ...
                            $.ajax({
                                type: "POST",
                                url: '<%=Url.Action("DeleteSelectedEnableDeviceException") %>',
                                data: { ids: ids },
                                dataType: "json",
                                success: function (data) {
                                    $.messager.alert('提示', data.d, 'info', function () {
                ...
                function DeleteEnableDeviceException(id) {
                    $.messager.confirm('确认', '确定要删除吗?', function (r) {
                        if (r) {
                            $.ajax({
                                type: "POST",
                                url: '<%=Url.Action("DeleteEnableDeviceException") %>',
                                data: { id: id },
                                dataType: "json",
                                success: function (data) {
                                    $.messager.alert('提示', data.d, 'info', function () {
                ...
                function AddEnableException() {
                    //var url = '<%=Url.Content("~/") %>demo/deviceexceptionadd?t=e';
                    var url = '<%=Url.Content("~/") %>demo/selectuser';
                    $('#wif')[0].src = url;
                    $('#w').window('open');
                    return false;
                }
                ...
                function addDeviceEnableAuthException() {
                    var o = {};
                    o.deviceid = window.ids;
                    var t = 'e';
                    o.t = t;

                    $.ajax({
                        type: "POST",
                        url: '<%=Url.Action("DeviceExceptionAddPost") %>',
                        data: o,
                        dataType: "json",
                        success: function (data) {
    ...
    <div id="w" class="easyui-window" title="&nbsp;" closed="true" modal="true" data-options="minimizable:false,collapsible:false,maximizable:false,onClose:function(){$('#wif').attr('src', ''); addDeviceEnableAuthException();return false}" style="width:850px;height:420px;padding:3px;">
