...
<%--<a id="btn_upload" onclick="javascript:return UnLockSelected();" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true">锁定</a>--%>
<a onclick="javascript:return UnLockSelected();" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-remove',plain:true">解除锁定</a>
     ...
     <table id="grid" class="easyui-datagrid" style="width:100%;border-top:1px solid #DDDDDD"
					data-options="url:'<%=Url.Content("~/Demo/GetDeviceUnLock") %>',idField:'deviceid',method:'get',displayMsg:'当前{from}-{to}，共 {total}',loadMsg:'正在加载...',rownumbers:true,checkOnSelect:false,selectOnCheck:true,singleSelect:false,pagination:true,border:false,fit:true,fitColumns:false,toolbar: '#tb',loadFilter: function (data) {
                            for (var i = 0; i < data.rows.length; ++i) {
                                data.rows[i].locktime2 = parseDate(data.rows[i].locktime);
                                data.rows[i].unlockime2 = parseDate(data.rows[i].unlocktime);
                                data.rows[i].deviceid2 = parseDate(data.rows[i].deviceid);
                                data.rows[i].deviceid3 = data.rows[i].deviceid;
                                data.rows[i].lockreason = '登陆密码错误';
                                data.rows[i].lockman = '管理员';
			...
			<tr><th data-options="field:'deviceid2',checkbox:true"></th><th data-options="field:'deviceid3',formatter: function(value,row,index){
				return '<a href=\'#\' onclick=\'javascript:return UnLock(&quot;'+value+'&quot;);\'>解除锁定</a>' + 
				...
				<thead>
					<tr>
                        ...
                        <th data-options="field:'deviceid'" width="270">设备号</th>
                        <th data-options="field:'model'" width="120">设备型号</th>
                        <th data-options="field:'description'" width="150">描述</th>
                        <th data-options="field:'locktime2'" width="120">锁定时间</th>
                        <th data-options="field:'lockreason'" width="120">锁定原因</th>
                        <th data-options="field:'lockman'" width="60">锁定人</th>
                        <th data-options="field:'unlocktime2'" width="120">解除锁定时间</th>
