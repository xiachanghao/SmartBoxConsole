...
<div id="_layout" class="easyui-layout" data-options="fit:true" style="">
    <div data-options="region:'north',split:false" style="height:32px;overflow:hidden;border:0px solid #DDDDDD;">
        <div class="easyui-panel cHead" data-options="" style="display:;font-size:12px;color:#528FB6;text-align: left; border:1px solid #DDDDDD;padding-left:5px;">
            <img src="../../themes/default/images/flexigrid/grid.png" /><span>用户审核>>禁用审核例外</span>
        </div>
...
<a id="btn_upload" onclick="javascript:return AddDisableException();" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true">添加需审核人员</a>
<a onclick="javascript:return DeleteSelected();" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-remove',plain:true">删除</a>
     ...
     <table id="grid" class="easyui-datagrid" style="width:100%;border-top:1px solid #DDDDDD"
					data-options="url:'<%=Url.Content("~/Demo/GetUserDisableAuthorizationException") %>',idField:'id',method:'get',displayMsg:'当前{from}-{to}，共 {total}',loadMsg:'正在加载...',rownumbers:true,checkOnSelect:false,selectOnCheck:true,singleSelect:false,pagination:true,border:false,fit:true,fitColumns:true,toolbar: '#tb',loadFilter: function (data) {
                ...
                return '<a href=\'#\' onclick=\'javascript:return DeleteDisableUserException(&quot;'+row.id+'&quot;);\'>删除</a>' + 
            ...
            <script type="text/javascript">
                ...
                function addUserDisableAuthException() {
                    var o = {};
                    o.useruid = window.ids;
                    var t = 'd';
                    o.t = t;
                ...
                function DeleteSelected() {
                    ...
                    $.messager.confirm('确认', '确定要删除所勾选的例外吗?', function (r) {
                        if (r) {
                            ...
                            $.ajax({
                                type: "POST",
                                url: '<%=Url.Action("DeleteSelectedDisableUserException") %>',
                                data: { ids: ids },
                                dataType: "json",
                                success: function (data) {
                                    $.messager.alert('提示', data.d, 'info', function () {
                ...
                function DeleteDisableUserException(id) {
                    $.messager.confirm('确认', '确定要删除吗?', function (r) {
                        if (r) {
                            $.ajax({
                                type: "POST",
                                url: '<%=Url.Action("DeleteDisableUserException") %>',
                                data: { id: id },
                                dataType: "json",
                                success: function (data) {
                                    $.messager.alert('提示', data.d, 'info', function () {
                ...
                function AddDisableException() {
                    //var url = '<%=Url.Content("~/") %>demo/userexceptionadd?t=d';
    ...
    <div id="w" class="easyui-window" title="&nbsp;" closed="true" modal="true" data-options="minimizable:false,collapsible:false,maximizable:false,onClose:function(){$('#wif').attr('src', ''); addUserDisableAuthException(); return false}" style="width:850px;height:420px;padding:3px;">
