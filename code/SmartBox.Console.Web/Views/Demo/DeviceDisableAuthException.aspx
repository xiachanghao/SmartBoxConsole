...
<div id="_layout" class="easyui-layout" data-options="fit:true" style="">
    <div data-options="region:'north',split:false" style="height:30px;overflow:hidden;border:0px solid #DDDDDD;">
        <div class="easyui-panel cHead" data-options="" style="display:;font-size:12px;color:#528FB6;text-align: left; border:1px solid #DDDDDD;padding-left:5px;">
            <img src="../../themes/default/images/flexigrid/grid.png" /><span>设备管理>>设备禁用审核例外</span>
        </div>
...
<a id="btn_upload" onclick="javascript:return AddDisableException();" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-add',plain:true">添加</a>
<a onclick="javascript:return DeleteSelected();" href="#" class="easyui-linkbutton" data-options="iconCls:'icon-remove',plain:true">删除</a>
     ...
     <table id="grid" class="easyui-datagrid" style="width:100%;border-top:1px solid #DDDDDD"
					data-options="url:'<%=Url.Content("~/Demo/GetDeviceDisableAuthorizationException") %>',idField:'id',method:'get',displayMsg:'当前{from}-{to}，共 {total}',loadMsg:'正在加载...',rownumbers:true,checkOnSelect:false,selectOnCheck:true,singleSelect:false,pagination:true,border:false,fit:true,fitColumns:true,toolbar: '#tb',loadFilter: function (data) {
                ...
                return '<a href=\'#\' onclick=\'javascript:return DeleteDisableDeviceException(&quot;'+row.id+'&quot;);\'>删除</a>' + 
            ...
            <script type="text/javascript">
                ...
                function DeleteSelected() {
                    ...
                    $.messager.confirm('确认', '确定要删除所勾选的例外吗?', function (r) {
                        if (r) {
                            ...
                            $.ajax({
                                type: "POST",
                                url: '<%=Url.Action("DeleteSelectedDisableDeviceException") %>',
                                data: { ids: ids },
                                dataType: "json",
                                success: function (data) {
                                    $.messager.alert('提示', data.d, 'info', function () {
                ...
                function DeleteDisableDeviceException(id) {
                    $.messager.confirm('确认', '确定要删除吗?', function (r) {
                        if (r) {
                            $.ajax({
                                type: "POST",
                                url: '<%=Url.Action("DeleteDisableDeviceException") %>',
                                data: { id: id },
                                dataType: "json",
                                success: function (data) {
                                    $.messager.alert('提示', data.d, 'info', function () {
                ...
                function AddDisableException() {
                    //var url = '<%=Url.Content("~/") %>demo/deviceexceptionadd?t=d';
