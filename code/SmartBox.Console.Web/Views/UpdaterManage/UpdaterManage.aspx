    ...
    <script type="text/javascript">
        ...
        $(document).ready(function() {
            ...
            var option = {
                height: gh,
                width: mainWidth,
                url: '<%=Url.Action("GetUpdaterInfo")%>',
                colModel: [
                    { display: '版本', name: 'Version', width: 50, sortable: false, align: 'left' },
                    { display: '版本说明', name: 'VersionSummary', width: 150, sortable: false, align: 'left' },
			        { display: '状态', name: 'VersionStatus1', width: 60, sortable: false, process: GetName, align: 'left' },
        ...
        function toolbarItem_onclick(cmd, grid) {
            if (cmd == "Add") {
                ...
                OpenModelWindow(url, { width: 750, height: maiheight - 100, caption: "升级程序发布向导", onclose: function() { $("#ManageVerList").flexReload(); } });
            }
            else if (cmd == "Delete") {
                var urls = '<%=Url.Action("DelAllVersions") %>';

                hiConfirm("你确定要删除该升级程序吗?", "提示", function(btn) {
                    if (btn == true) {
...
<body>
 <% using (Html.BeginForm("GetDownLoadFile", "UpdaterManage", FormMethod.Post, new { id = "fmEdit" }))
    ...
    <div style="padding: 1px;">
        <div class="cHead">
            <div class="ftitle">
                <span id="departmentName">更新程序管理</span>
            </div>
        </div>
