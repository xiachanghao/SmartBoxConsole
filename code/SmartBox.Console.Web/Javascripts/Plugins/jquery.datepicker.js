...
(function($) {
    ...
    $.fn.datepicker = function(o) {
        ...
        $.extend(def, o);
        $.extend(DateTimeoptions, o);
        $.DateTimeMask =
        {
            //获得焦点时候的处理函数
            SetFocus: function(obj, DateTimeoptions) {
            ...
            //失去焦点时候的处理函数
            LostFocus: function(obj, DateTimeoptions) {
                var ls_date, ls_time;
                var lb_error = false;
                ls_date = obj.value;
                if (ls_date != "") {
                    if (ls_date == "0000-00-00") {
                        if (!DateTimeoptions.isnull) lb_error = true;
                    }
                    else {
                        if (DateTimeoptions.masktype == "6" && ls_date.length == 7) {
                            ls_date += "-01";
                        }
                ...
                if (lb_error) {
                    if (!DateTimeoptions.lawlessmessage || DateTimeoptions.lawlessmessage != "") {
                        alert(DateTimeoptions.lawlessmessage);
                        obj.value = "";
                    }
                }
            ...
            //按键时候的处理函数
            KeyDown: function(objTextBox, event, DateTimeoptions) {
                //按键常量
                var KEY = {
                    BACKSPACE: 8,
                    TAB: 9,
                ...
                switch (nKeyCode) {
                    ...
                    case KEY.ENTER:
                        ...
                        if (DateTimeoptions.EnterMoveToNext) event.keyCode = 9;
                        return;
                }
                //只读就不执行任何操作
                ...
                switch (nKeyCode) {
                    case KEY.BACKSPACE:
                        if (strText != "")
                            strText = strText.substr(0, nTextLen - 1);
                        break;
                    default:
                        if (nCursorPos == 10) break;
                        ...
                        if (behindtext != "-" && behindtext != ":" && behindtext != " ") {
                            ...
                            if (!$.DateTimeMask.DealWith(DateTimeoptions.masktype, preText, nCursorPos)) break;
                            strText = preText;
                            nCursorPos++;
                        }
            ...
            DealWith: function(masktype, input, nCursorPos) {
                ...
                var yearlen = ls_year.length;
                if (ls_year.length < 4) {
                    for (var i = 0; i < 4 - yearlen; i++) {
                        ls_year += "1";
                    }
                }
                ls_month = input.substr(5, 2);
                var monthlen = ls_month.length;
                if (ls_month.length < 2) {
                    for (var i = 0; i < 2 - monthlen; i++) {
                        ls_month += "1";
                    }
                }
                ls_day = input.substr(8, 2);
                var daylen = ls_day.length;
                if (ls_day.length == 0) {
                    ls_day += "01";
                }
                else if (ls_day.length == 1) {
                    if (ls_day == "0") {
                        ls_day += "1";
                    } else {
                        ls_day += "0";
                ...
                return ($.DateTimeMask.isValidDate(ls_date))
                return true;
            },
            ...
            isValidDate: function(strDate) {
                var ls_regex = "^(?:(?!0000)[0-9]{4}-(?:(?:0[1-9]|1[0-2])-(?:0[1-9]|1[0-9]|2[0-8])|(?:0[13-9]|1[0-2])-(?:29|30)|(?:0[13578]|1[02])-31)|(?:[0-9]{2}(?:0[48]|[2468][048]|[13579][26])|(?:0[48]|[2468][048]|[13579][26])00)-02-29)$";
            ...
            getDateTime: function(DateTimeoptions) {
                var d = new Date();
                ...
                switch (DateTimeoptions.masktype) {
                    case "1":
                        return DateTimeoptions.isnow ? (ls_date + " " + ls_time) : (vYear + "-" + vMon + "-" + "01 00:00:00");
                    case "2":
                        return DateTimeoptions.isnow ? (ls_date + " " + ls_time.substr(0, 5)) : (vYear + "-" + vMon + "-" + "01 00:00");
                    case "3":
                        return DateTimeoptions.isnow ? ls_date : (vYear + "-" + vMon + "-" + "01");
                    case "4":
        ...
        //$(this).val($.DateTimeMask.getDateTime(DateTimeoptions));

        $(this).keydown(function(event) {
            $.DateTimeMask.KeyDown(this, event, DateTimeoptions);
        });

        $(this).focus(function() {
            $.DateTimeMask.SetFocus(this, DateTimeoptions);
            DateTimeoptions.whenfocus();
        });
        $(this).blur(function() {

            if (!$.DateTimeMask.LostFocus(this, DateTimeoptions)) {
                if (!DateTimeoptions.whenblur(this.value)) this.value = this.oldvalue;
            }
        });
