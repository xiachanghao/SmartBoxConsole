...
function CloseModelWindow(callback, dooptioncallback) {
    if ($.closeIfrm)
        $.closeIfrm(callback, dooptioncallback);
    else
...
function CompareTime(time1, time2) {  //14：30    15：00
    var bstr = time1.split(":");
    var bm = parseInt(bstr[0], 10);
    var bs = parseInt(bstr[1], 10);
    var estr = time2.split(":");
    var em = parseInt(estr[0], 10);
    var es = parseInt(estr[1], 10);
    if (bm > em || (bm == em && bs > es)) {
        return false;
    }
    return true;
} 
function SetInputDisabled(id, disabled) {
    ...
    if (disabled) {
        ...
        obj.addClass("disabledcolor");
    }
    else {
        obj.attr("disabled", false);
        obj.removeClass("disabledcolor");
    }
}
...
function HiddenSelect() {
    var v = getiev();
    if (v == 6) {
        $("body").addClass("hiddenselect");
    }
}
function ShowSelect() {
    var v = getiev();
    if (v == 6) {
        $("body").removeClass("hiddenselect");
    }
}
function OpenModalDialog(url, packurl, title, obj, features) {
    var turl = packurl + "/?Url=" + encodeURIComponent(url) + "&Title=" + escape(title);
    return window.showModalDialog(turl, obj, features);
}
function showPrompt(message, isError, right) {
    var ht = [], html, css, rt, msg;
    if (message)
        msg = message;
    else {
        if (isError)
            msg = "非常抱歉，无法执行您的操作，请稍后再试";
        else
            msg = "正在操作,请稍后...";
    }
    if (isError)
        css = isError == true ? "loaderror" : "loadicon";
    else
        css = "loadicon";
    if (right)
        rt = right.toString() + "px";
    else
        rt = "15" + "px";
    html = $("#promptpannel");
    if (html.length > 0) {
        html.html(msg).attr("class", "ptogtitle " + css + "").css("right", rt);
        html.show();
    }
    else {
        ht.push('<div id="promptpannel" class="ptogtitle ', css, ' " ');
        ht.push(' style="display:none">');
        ht.push(msg);
        ht.push('</div>');
        $("body").append(ht.join(''));
        $("#promptpannel").css("right", rt).show();
    }
}

function hidePrompt(delayTime) {
    if (delayTime)
        var timeName = setTimeout(function() { $("#promptpannel").hide(); clearTimeout(timeName); }, delayTime);
    else
        $("#promptpannel").hide(); 
}
